document.addEventListener("DOMContentLoaded", () => {
  ...
  const userMessage = chatHistory[chatId].messages.slice(-1)[0].content;
  ...
  try {
    ...
    // Display AI message placeholder
    const messageDiv = document.createElement("div");
    ...
    // Final rendering with copy buttons and syntax highlighting
    messageDiv.remove();
    addFormattedMessageToUI("ai", resultText);
    // typeCharacterByCharacter(".message.ai .message-content", resultText);
    return resultText;
...
function typeCharacterByCharacter(selector, text, speed = 50, callback = null) {
  ...
  function createNewChat() {
    ...
    messagesContainer.innerHTML = `
            <div class="intro-message">
                <h1>Welcome to GenAI Chatbot</h1>
                <p>Ask me anything. I'm powered by Llama 2.</p>
  ...
  function exportCurrentChat() {
    ...
    chat.messages.forEach((message) => {
      const role = message.role === "user" ? "You" : "GenAI Chatbot";
      exportText += `## ${role}:\n${message.content}\n\n`;
    });
